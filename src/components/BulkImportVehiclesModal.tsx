...
interface BulkImportVehiclesModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export const BulkImportVehiclesModal: React.FC<BulkImportVehiclesModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const parseCSV = (text: string) => {
    ...
    // Assume columns: Vehicle Model,WB Plate Number,Assigned Driver / Owner,Seating Capacity,Vehicle Type,Fuel Type,Color
    const data = [];
    let validCount = 0;
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (!veh.model || !veh.plate || !veh.owner) {
        veh.isValid = false;
        veh.errorMsg = 'Missing required fields';
      }
      
      if (veh.isValid) validCount++;
      data.push(veh);
    }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success('Bulk Import Successful', `Successfully imported ${result.count} vehicles.`);
        
        // Update local storage so the frontend updates immediately
        const currentVehicles = storage.getVehicles();
        validData.forEach(d => {
           currentVehicles.unshift({
             id: 'veh-import-' + Math.random().toString(36).substr(2, 9),
             userId: 'admin',
             model: d.model,
             registrationNumber: d.plate,
             seatingCapacity: parseInt(d.capacity) || 4,
             status: 'approved',
             driverName: d.owner,
             vehicleType: d.vType || 'Sedan',
             fuelType: d.fuel || 'Petrol'
           });
        });
        storage.setVehicles(currentVehicles);
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between p-6 border-b border-slate-800 bg-slate-900/50">
          <div>
            ...
            <p className="text-sm text-slate-400 mt-1">Upload a CSV file to register multiple corporate vehicles.</p>
          </div>
          ...
          {!file ? (
            ...
            >
              ...
              <p className="text-slate-400 text-sm mt-1 max-w-md">
                Ensure your CSV has columns: Vehicle Model, WB Plate Number, Assigned Driver, Seating Capacity, Vehicle Type, Fuel Type, Color
              </p>
              <input 
                type="file" 
                accept=".csv" 
                      ...
                      <table className="w-full text-left text-xs">
                        <thead className="bg-slate-800 sticky top-0">
                          <tr>
                            ...
                            <th className="px-4 py-2 font-semibold text-slate-300">Vehicle</th>
                            <th className="px-4 py-2 font-semibold text-slate-300">Plate Number</th>
                            <th className="px-4 py-2 font-semibold text-slate-300">Driver</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-700/50 bg-slate-900/50">
                          {parsedData.slice(0, 100).map((row, idx) => (
                            <tr key={idx}>
                              ...
                              <td className="px-4 py-2 text-cyan-400 font-mono font-bold">{row.plate}</td>
                              <td className="px-4 py-2 text-slate-300">{row.owner}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
